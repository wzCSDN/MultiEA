...
class Experiment:
    ...
    def evaluate_many(self, it,file_num,test, ins_emb, mapping_emb=None):
        # r = 0.2
        ...
        # top_k = [1, 3, 5, 10]
            ...
            # distance_1=-(r*distance_AB+((1-r)/2)*AB_C+((1-r)/2)*AB_D)
            # distance_2=-(r*distance_AC+((1-r)/2)*AC_B+((1-r)/2)*AC_D)
            # distance_3=-(r*distance_AD+((1-r)/2)*AD_B+((1-r)/2)*AD_C)
            # r1 =0.2
            # r2 =0.4
            # r3 =0.4
            r1 =1
            ...
            r3 =0
            #boot!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            distance_1=-(r1*distance_AB+r2*AB_C+r3*AB_D)
            distance_2=-(r1*distance_AC+r2*AC_B+r3*AC_D)
            distance_3=-(r1*distance_AD+r2*AD_B+r3*AD_C)

            # distance_1= -sim(emb_A, emb_B, metric=self.args.test_dist, normalize=True, csls_k=self.args.csls)
            # distance_2= -sim(emb_A, emb_C, metric=self.args.test_dist, normalize=True, csls_k=self.args.csls)
            # distance_3= -sim(emb_A, emb_D, metric=self.args.test_dist, normalize=True, csls_k=self.args.csls)
        ...
        if file_num==3:
            ...
            # r3 =0.2
            # boot!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            distance_1 = -(r1 * distance_AB + r2 * AB_C)
            distance_2 = -(r1 * distance_AC + r2 * AC_B)
    ...
    def init_emb(self):
        ...
        if self.args.IsLaBSE:
            self.ins_embeddings=torch.tensor(np.load("Mydata/ent_LaBSE.txt",allow_pickle=True))
        ...
        if self.args.decoder == ["rotate"] or self.args.decoder == ["hake"]:
            ...
            if args.IsLaBSE:
                nn.init.xavier_normal_(self.ins_embeddings)
            else:
    ...
    def train_and_eval(self):
        ...
        if args.IsLaBSE:
            params = nn.ParameterList([self.rel_embeddings.weight] +
        ...
        for it in range(0, self.args.epoch):
                ...
                if k_d.name=="manyalign":
                    if args.IsLaBSE:
                        loss = self.train_1_epoch(it, opt, graph_encoder, k_d, d.ins_G_edges_idx, d.triple_idx,
                                              ...
                                              d.boot_pair_dix, self.ins_embeddings, self.rel_embeddings.weight)
                    else:
                        if args.file_num==4:
                            loss = self.train_1_epoch(it, opt, graph_encoder, k_d, d.ins_G_edges_idx, d.triple_idx,
                                              d.ill_train_idx, [d.kg1_ins_ids, d.kg2_ins_ids,d.kg3_ins_ids,d.kg4_ins_ids], d.boot_triple_idx,
            ...
            if (it + 1) % self.args.check == 0:
                ...
                with torch.no_grad():
                    ...
                    #     np.save(self.save + "/%s_ins.npy" % (name_str), emb)
                    ...
                    if len(d.ill_val_idx) > 0:
                        ...
                        if args.my:
                            ...
                            result = self.evaluate_many(it,args.file_num,d.ill_test_idx, emb, self.mapping_ins_emb)
                        else:
                            result = self.evaluate(it, d.ill_test_idx, emb, self.mapping_ins_emb)
                    test_id=d.ill_test_idx

                    # visual
                    # best_id = np.array([test_id[i] for i in result[2]])
                    # best_emd = np.concatenate(
                    #     [emb[best_id[:, 0]], emb[best_id[:, 1]], emb[best_id[:, 2]]], axis=0)
                    # np.save("dwy.npy", best_emd)
                    # best_emd = np.concatenate(
                    #     [emb[best_id[:, 0]], emb[best_id[:, 1]], emb[best_id[:, 2]], emb[best_id[:, 3]]], axis=0)
                    # np.save("dbp.npy", best_emd)
                ...
                #             acc, mean = self.best_result
                #             logger.info(" acc of top {} = {}, meanrank = {:.3f}".format(top_k, acc, mean))
                #             logger.info("acc of top {} = {}, meanrank = {:.3f} \n".format(top_k, acc, mean))
                #         else:
                #             logger.info("Early stop, best result:")
    ...
    def train_1_epoch(self, it, opt, encoder, decoder, edges, triples, ills, ids, boot_triples, boot_pairs, ins_emb, rel_emb):
        ...
        if self.args.train_batch_size == -1:
            ...
            train_batch_size = self.args.train_batch_size        #这块不用改，4500*2
...
if __name__ == '__main__':
    ...
    # BaseLines
    # parser.add_argument("--decoder", type=str, default="TransE,MTransE_Align_Many", nargs="?", help="which decoder to use: . min = 1")
    # parser.add_argument("--k", type=str, default="20,20", help="negtive sampling number for each decoder")
    # parser.add_argument("--margin", type=str, default="1,1", help="margin for each margin based ranking loss (or params for other loss function)")
    # parser.add_argument("--alpha", type=str, default="1,50", help="weight for each margin based ranking loss")
    # parser.add_argument("--sampling", type=str, default="Multi-N,Multi-N", help="negtive sampling method for each decoder")
    # parser.add_argument("--decoder", type=str, default="ManyAlign", nargs="?", help="which decoder to use: . min = 1")
    # parser.add_argument("--k", type=str, default="20", help="negtive sampling number for each decoder")
    ...
    # parser.add_argument("--alpha", type=str, default="50", help="weight for each margin based ranking loss")
    # parser.add_argument("--sampling", type=str, default="Multi-N", help="negtive sampling method for each decoder")
    #
    # parser.add_argument("--encoder", type=str, default="", nargs="?", help="which encoder to use: . max = 1")
    ...
    # parser.add_argument("--data_dir", type=str, default="Mydata", required=False)
    ...
    parser.add_argument("--evaluate_many", type=bool, default=False)
    parser.add_argument("--self_loops", type=bool, default=True)
    parser.add_argument("--data_dir", type=str, default="Mydata/DWY", required=False)
    parser.add_argument("--file_num",type=int,default=3)

    # parser.add_argument("--data_dir", type=str, default="Mydata/Pre_data", required=False,
    ...
    # parser.add_argument("--train_batch_size", type=int, default=10, help="train batch_size (-1 means all in)")
    ...
    parser.add_argument("--hiddens", type=str, default="256,256,256", help="hidden units in each hidden layer(including in_dim and out_dim), splitted with comma")
    ...
    # Load Data
    d = AlignmentData(data_dir=args.data_dir, file_num=args.file_num, rate=args.rate, share=args.share, swap=args.swap, val=args.val, with_r=args.encoder.lower()=="naea")

    # if args.save != "":
    #     if not os.path.exists(args.save):
    #         os.makedirs(args.save)
    #
    #     np.save("test_label_dwy.npy", d.ill_test_idx)
